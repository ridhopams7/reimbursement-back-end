...
const _fileReimbursementHandler = (file: any, options: any): any => {
  if (!file) throw new Error('no file');
  ...
  fs.mkdirpSync(`${path}${options.modules}/`);

  fs.mkdirpSync(`${path}${options.modules}/${options.folderCode}`);

  path = `${path}${options.modules}/${options.folderCode}`;

  const filename =
    `${options.folderCode}-${options.date.getTime()}${getExtention(file.hapi.headers['content-type'])}`;
  path = `${path}/${filename}`;
  const fileStream = fs.createWriteStream(path);
...
export const uploader = (file: any, options: any): any => {
  ...
  return _fileReimbursementHandler(file, options);
};